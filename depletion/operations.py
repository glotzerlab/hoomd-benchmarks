...
def add_compression(project):
    ...
    def compress(job):
        ...
        with job:
            ...
            phi_p_ini = 0.2
            n = sp['n']
            L_target = n*(1.0/phi)**(1./3.)

            system = hoomd.init.create_lattice(n=n, unitcell=hoomd.lattice.sc(a=(1/phi_p_ini)**(1./3.),type_name='Poly'))
            system.particles.types.add('Depletant')
            ...
            while L_target < L:
                ...
                if c.device.comm.rank == 0:
                    print("phi = {:.3f}: overlaps = {} ".format(((n*n*n) / (L*L*L)), overlaps), end = '');
    ...
    def benchmark(job):
        ...
        with job:
            ...
            if c.device.mode == 'gpu':
                ...
                hoomd.run(2000, limit_hours=20.0/3600.0)
